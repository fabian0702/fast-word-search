...
public:
    TrigramQuery() : mapping("mapping.bin"), index("index.bin"), offsets("offsets.bin") {}
    ...
    std::vector<uint32_t> query(const std::string &query_string, const MemoryMappedFile<uint8_t> &input_file);
};
